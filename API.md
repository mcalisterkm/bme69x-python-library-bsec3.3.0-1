...
For the guided workflow and concepts, see [Documentation.md](Documentation.md).
...
- `get_bsec_data()` -> dict | None
  ...
  - Returns a dict with keys such as `sample_nr`, `timestamp`, `iaq`, `iaq_accuracy`, `temperature`, `raw_temperature`, `humidity`, `raw_humidity`, `raw_gas`, `static_iaq`, `co2_equivalent`,  `comp_gas_value`, etc.
  - Returns Python `None` when no new BSEC-processed output is available (sensor not ready / polled too frequently).
