...
const char *bsec_conf_path = "bsec_v3-3-0-1/release_bin/Sel_IAQ/config/bme690/bme690_sel_33v_3s_4d/bsec_selectivity.config";
FILE *bsec_conf;
#endif
...
{
    uint8_t chip_id = 0;
    int8_t rslt = bme69x_get_regs(BME69X_REG_CHIP_ID, &chip_id, 1, &(self->bme));
    if (rslt < BME69X_OK)
    {
        PyErr_SetString(bmeError, "Failed to read chip ID register");
        return NULL;
    }

    self->bme.chip_id = chip_id;
    return Py_BuildValue("i", (int)chip_id);
}
...
static PyObject *bme_open_i2c(BMEObject *self, PyObject *args)
{
    ...
    if ((uint8_t)size == 1)
    {
        ...
        else if (ioctl(self->linux_device, I2C_SLAVE, i2c_addr) < 0)
        {
