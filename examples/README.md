...
load_config_from_path.py shows how to support command line parameters for the main variables such as I2C address,I2C Bus,  Mode, and path to .config. You need to create config and state files before you can load them, see Burn-In above and note what your saved file names are and edit the paths in this file. 

## Parallel_model & Parallel_mode_ulp
parallel_mode.py uses the default temperature and duration profile used in AI Studio. The parallel part is the iphysical environment sensors (temp, Humidity, pressure) are read in parallel with the heater plate.  Serial mode runs the environmental sensors first before starting up the heater plate. 
